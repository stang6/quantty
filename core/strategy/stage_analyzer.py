...
class StageAnalyzer:
    ...
    def _analyze_stage(self):
        ...
        if last_close > self.current_wma:
            if self.wma_slope_trend == "UP":
                # Price above WMA, WMA is rising (Stage 2 - Uptrend/Buy Zone)
                self.current_stage = "STAGE 2 (Uptrend)"
            elif self.wma_slope_trend == "FLAT":
                # Price above WMA, WMA is flat (Stage 1 - Accumulation/Breakout)
                self.current_stage = "STAGE 1 (Accumulation/Breakout)"
            else: # WMA is DOWN
                # Price above WMA, WMA is falling (Potential transition from Stage 4)
        ...
        # 2. Price is Below WMA
            ...
            if self.wma_slope_trend == "DOWN":
                # Price below WMA, WMA is falling (Stage 4 - Downtrend/Avoid Zone)
                self.current_stage = "STAGE 4 (Downtrend)"
            elif self.wma_slope_trend == "FLAT":
                # Price below WMA, WMA is flat (Stage 3 - Distribution/Breakdown)
                self.current_stage = "STAGE 3 (Distribution/Breakdown)"
            else: # WMA is UP
                # Price below WMA, WMA is rising (Potential transition from Stage 2)
                self.current_stage = "STAGE 3 (Distribution/Transition)"
                
        logger.info(f"[{self.symbol}] Determined Stage: {self.current_stage}")

    def get_analysis_summary(self) -> Dict:
        """Returns the current analysis summary."""
        if self.data.empty:
            return {
                "symbol": self.symbol,
                "error": True,
